...
class Tiket extends CI_Controller {
	...
	public function index(){
		...
		echo $this->session->userdata('asal');
		// die();
	...
	public function cekjadwal($tgl='' , $asl='', $tjn=''){
		...
		$data['jadwal'] = $this->db->query("SELECT * FROM tbl_jadwal LEFT JOIN tbl_bus on tbl_jadwal.kd_bus = tbl_bus.kd_bus LEFT JOIN tbl_tujuan on tbl_jadwal.kd_tujuan = tbl_tujuan.kd_tujuan WHERE tbl_jadwal.wilayah_jadwal ='".$tujuan."'")->result_array();
		// die(print_r($data));
	...
	public function beforebeli($jadwal="",$asal='',$tanggal=''){
		...
		if ($this->session->userdata('username')){
			$id = $this->session->userdata('jadwal');
	...
	public function afterbeli(){
		...
		$data['tglberangkat'] = $this->session->userdata('tanggal');
		if ($data['kursi']) {
		// die(print_r($data));
	...
	public function gettiket($value=''){
		...
		$tanggaljadwal = $this->session->userdata('tanggal');
		...
		for($i=0; $i<$count; $i++) {
			$simpan = array(
				'kd_order' => $getkode,
				'kd_tiket' => 'T'.$getkode.$kd_jadwal.$asal['kd_tujuan'].str_replace('-', '', $tanggaljadwal).$kursi[$i],
				'kd_jadwal'	=> $kd_jadwal,
				'kd_pelanggan' => $kd_pelanggan,
				'asal_order' => $asal['kd_tujuan'],
				'nama_order'	=> $nama_pemesan,
				'tgl_beli_order'	=> $tanggal,
				'tgl_berangkat_order' => $tanggaljadwal,
	...
	public function checkout($value=''){
        ...
        $config = [
               ...
               'smtp_pass' => 'xxxxx',      // Password gmail kamu
               'smtp_port' => 465,
               'crlf'      => "rn",
               'newline'   => "rn"
           ];
